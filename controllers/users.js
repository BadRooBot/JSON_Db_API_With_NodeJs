...
export const writeUser  =async (req,res)=>{   //to create New User or SignUp//لانشاء مستخدم جديد او انشاء حساب جديد +_+
    try {
        ...
        "follwos":['Abo Sh3rif','Abo Sherif','badroobot','mesho'],
        "follwoing":['Abo Sh3rif','Abo Sherif','badroobot','mesho'],
        "requestGroup":[{'requestId':'1','email':'b@b.b','groupId':'1'}],
        "points":0,
        "isVIP":false,
        "online":false
    };
    ...
    /*
        ...
        //writeStreamData.end('');*/
        res.status(200).json({userId,name,email,password,profileImage});
    }
    } catch (error) {
        console.log(error);
        res.status(500).json({message:error});
